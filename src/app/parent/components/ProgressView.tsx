                ...
                <div key={ann.id} style={{ borderRadius: "12px", border: `1.5px solid ${priColor}22`, background: `${priColor}07`, padding: "0.9rem 1.1rem", borderLeft: `4px solid ${priColor}` }}>
                  ...
                  {ann.image_url && (
                    <div style={{ marginTop: "0.6rem" }}>
                      <img
                        src={ann.is_sanity && ann.image_url ? urlFor(ann.image_url).width(500).url() : ann.image_url}
                        alt={ann.title}
                ...
                <div key={report.id} className="portal-card">
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.5rem", borderBottom: "1px solid var(--color-gray-150)", paddingBottom: "1rem" }} className="flex-wrap gap-2">
                    ...
                    <div style={{ display: "flex", gap: "0.5rem" }}>
                      <button className="btn-portal-outline" style={{ padding: "0.5rem 1.15rem", fontSize: "0.8rem", display: "flex", gap: "0.5rem", alignItems: "center" }} onClick={() => setPrintReport(report)}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
                        <span>Cetak Rapor PDF</span>
                      </button>
                      {(() => {
                        const existingCert = certificates.find(
                          (c) => c.report_id === report.id || (selectedChild && c.student_id === selectedChild.id && c.module_name?.toLowerCase() === report.module_name?.toLowerCase())
                        );
                        if (existingCert) {
                          return (
                            <a
