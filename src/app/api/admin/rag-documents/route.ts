...
export async function GET() {
  try {
    ...
    const { data: docs, error } = await supabase.from("rag_documents").select("*").order("updated_at", { ascending: false });
...
export const POST = withAdminAuth(async (request: any) => {
  try {
    ...
    if (!title || !content) return NextResponse.json({ error: "Judul dan isi dokumen wajib diisi." }, { status: 400 });
    const doc = await upsertRagDocument({ title, content, source: source || "manual", metadata: metadata || {} });
...
export const PATCH = withAdminAuth(async (request: Request) => {
  try {
    ...
    if (!id || !title || !content) return NextResponse.json({ error: "ID, judul, dan isi dokumen wajib diisi." }, { status: 400 });
    const doc = await upsertRagDocument({ id, title, content, source: source || "manual", metadata: metadata || {} });
    return NextResponse.json({ data: doc });
  } catch (error: any) {
