...
export async function GET(request: NextRequest) {
  try {
    ...
    const [regularBytes, semiBoldBytes, boldBytes] = await Promise.all([
      fs.promises.readFile(path.join(fontsDir, "Montserrat-Regular.ttf")),
      fs.promises.readFile(path.join(fontsDir, "Montserrat-SemiBold.ttf")),
      fs.promises.readFile(path.join(fontsDir, "Montserrat-Bold.ttf")),
    ]);
