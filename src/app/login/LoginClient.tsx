...
import LoginRolePicker from "./LoginRolePicker";
import "./login.css";

declare global {
  interface Window {
    FB?: { getLoginStatus: (callback: (response: { status: string }) => void) => void; };
  ...
  return (
    <div className="auth-wrapper">
      ...
      <button onClick={toggleTheme} className="auth-theme-toggle" aria-label="Toggle theme">
        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transform: theme === "dark" ? "rotate(40deg)" : "rotate(0deg)", transition: "transform 0.5s cubic-bezier(0.4, 0, 0.2, 1)", overflow: "visible" }} className="theme-toggle-icon">
          <mask id="login-moon-mask">
            <rect x="0" y="0" width="100%" height="100%" fill="white" />
            <circle cx={theme === "dark" ? "12" : "30"} cy={theme === "dark" ? "4" : "0"} r="8" fill="black" style={{ transition: "cx 0.5s cubic-bezier(0.4, 0, 0.2, 1), cy 0.5s cubic-bezier(0.4, 0, 0.2, 1)" }} />
          </mask>
          <circle cx="12" cy="12" r={theme === "dark" ? "9" : "5"} fill="currentColor" mask="url(#login-moon-mask)" style={{ transition: "r 0.5s cubic-bezier(0.4, 0, 0.2, 1)" }} />
          <g stroke="currentColor" style={{ opacity: theme === "dark" ? 0 : 1, transform: theme === "dark" ? "scale(0.5)" : "scale(1)", transformOrigin: "center", transition: "opacity 0.5s ease, transform 0.5s cubic-bezier(0.4, 0, 0.2, 1)" }}>
      ...
      <div className="auth-card">
        ...
        <LoginRolePicker role={role} setRole={setRole} />

        <div className="auth-tabs">
          <button type="button" onClick={() => { setIsRegister(false); setErrorBanner(""); setSuccessBanner(""); }} className={`auth-tab ${!isRegister ? "active" : ""}`}>Portal Login</button>
          <button type="button" onClick={() => { setIsRegister(true); setErrorBanner(""); setSuccessBanner(""); }} className={`auth-tab ${isRegister ? "active" : ""}`}>Daftar Sekarang</button>
        </div>
        ...
        <form onSubmit={isRegister ? handleRegister : handleLogin} className="space-y-4">
            ...
            <>
              <div className="form-group">
                ...
                <div className="input-with-icon">
                  ...
                  <input type="text" id="name-input" className="form-input" placeholder="Masukkan Nama Lengkap Anda" required value={fullName} onChange={(e) => setFullName(e.target.value)} disabled={loading} />
                </div>
              </div>
              ...
              <div className="form-group">
                ...
                <div className="input-with-icon">
                  ...
                  <select id="role-input" className="form-input" value={role} onChange={(e) => setRole(e.target.value)} disabled={loading} style={{ cursor: "pointer" }}>
          ...
          <div className="form-group">
            ...
            <div className="input-with-icon">
              ...
              <input type="email" id="email-input" className="form-input" placeholder="nama@email.com" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={loading} />
            </div>
          </div>
          ...
          <div className="form-group">
            ...
            <div className="input-with-icon">
              ...
              <input type="password" id="password-input" className="form-input" placeholder="••••••••" required autoComplete={isRegister ? "new-password" : "current-password"} value={password} onChange={(e) => setPassword(e.target.value)} disabled={loading} />
            </div>
          </div>

          <button type="submit" className="form-btn" disabled={loading}>
            <span>{loading ? "Menghubungkan..." : isRegister ? "Daftar Sekarang" : "Portal Login"}</span>
          </button>
        </form>

        <div className="auth-divider"><span>atau</span></div>

        <button type="button" onClick={handleGoogleLogin} className="google-login-btn" disabled={loading}>
          <svg className="google-icon" viewBox="0 0 24 24" width="20" height="20" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" fill="#EA4335"/></svg>
          <span>Masuk / Daftar dengan Google</span>
        </button>

        <button type="button" onClick={handleFacebookLogin} className="facebook-login-btn" disabled={loading}>
          <svg className="facebook-icon" viewBox="0 0 24 24" width="20" height="20" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
