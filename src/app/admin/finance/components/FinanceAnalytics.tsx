...
import {
  useFinanceAnalyticsData,
  Student,
  Payment,
  TooltipState,
  ChartDataPoint,
} from "../hooks/useFinanceAnalyticsData";
...
export default function FinanceAnalytics({
  ...
  const [tooltip, setTooltip] = useState<TooltipState>({
    show: false, x: 0, y: 0, monthName: "", expected: 0, collected: 0,
  });
  ...
  } = useFinanceAnalyticsData(students, allPayments, sppPrices, selectedMonth);
  ...
  const scaleY = (val: number): number => 220 - (val / maxVal) * 180;
  ...
  const handleHoverBar = (e: React.MouseEvent<SVGRectElement>, d: ChartDataPoint) => {
    const rect = (e.target as SVGRectElement).getBoundingClientRect();
    const container = (e.target as SVGRectElement).ownerSVGElement!.getBoundingClientRect();
    setTooltip({
      show: true,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: "1.5rem" }} className="report-detail-layout">
        <div className="portal-card" style={{ padding: "1.5rem", position: "relative" }}>
          ...
          <div style={{ position: "relative", width: "100%" }}>
            ...
            {tooltip.show && (
              <div style={{ position: "absolute", left: `${tooltip.x}px`, top: `${tooltip.y}px`, transform: "translate(-50%, -100%)", backgroundColor: "rgba(15, 23, 42, 0.95)", color: "white", padding: "8px 12px", borderRadius: "8px", fontSize: "0.75rem", pointerEvents: "none", zIndex: 100, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.3)", border: "1px solid rgba(255, 255, 255, 0.15)", display: "flex", flexDirection: "column", gap: "3px" }}>
        ...
        <div className="portal-card" style={{ padding: "1.5rem", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ position: "relative", width: "160px", height: "160px", margin: "1.5rem 0" }}>
            <svg width="160" height="160" viewBox="0 0 160 160" style={{ transform: "rotate(-90deg)" }}>
              ...
              {pPaid > 0 && (
                <circle cx="80" cy="80" r="50" fill="none" stroke="#10b981" strokeWidth="16" strokeDasharray={strokeDashLunas} strokeDashoffset="0" style={{ transition: "stroke-dasharray 0.5s ease" }} />
              )}
              {pPending > 0 && (
                <circle cx="80" cy="80" r="50" fill="none" stroke="#f59e0b" strokeWidth="16" strokeDasharray={strokeDashPending} strokeDashoffset={offsetPending} style={{ transition: "stroke-dasharray 0.5s ease, stroke-dashoffset 0.5s ease" }} />
              )}
              {pUnpaid > 0 && (
                <circle cx="80" cy="80" r="50" fill="none" stroke="#ef4444" strokeWidth="16" strokeDasharray={strokeDashUnpaid} strokeDashoffset={offsetUnpaid} style={{ transition: "stroke-dasharray 0.5s ease, stroke-dashoffset 0.5s ease" }} />
              )}
            </svg>
            ...
            return (
              <div key={prog.name} style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                ...
                <div style={{ width: "100%", height: "14px", backgroundColor: "#f1f5f9", borderRadius: "10px", overflow: "hidden", position: "relative" }}>
                  <div style={{ width: `${Math.min(100, progressRatio)}%`, height: "100%", backgroundColor: prog.color, borderRadius: "10px", transition: "width 0.5s ease" }} />
                  <span style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", fontSize: "0.65rem", fontWeight: "900", color: progressRatio > 80 ? "white" : "var(--color-gray-600)" }}>
