...
/* ── SVG Shape Dekoratif (diadaptasi dari awwaltech.com) ── */
const ShapeDown = () => (
  <svg
        ...
        <div className={styles.processGrid}>
          {PROCESS_STEPS.map((item) => (
            <div key={item.step} className={styles.processCardWrapper}>
              ...
              {/* Kartu putih — icon di dalam, posisi atas atau bawah sesuai tipe */}
              <article className={styles.processCard}>

                {/* Icon di ATAS konten (kartu genap: 02, 04, 06, 08) */}
                ...
                {/* Icon di BAWAH konten (kartu ganjil: 01, 03, 05, 07) */}
                {item.iconPosition === "bottom" && (
                  <div className={styles.iconBadge}>{item.icon}</div>
                )}
              </article>

              {/* SVG Shape dekoratif — langsung menyambung di bawah kartu */}
