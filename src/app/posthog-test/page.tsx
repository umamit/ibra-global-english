...
export default function PostHogTestPage() {
  ...
  const handleCaptureEvent = () => {
    ...
    try {
      ...
      setStatus(`Gagal mengirim event: ${error instanceof Error ? error.message : String(error)}`);
    }
  };
  ...
  const handleSentryMetric = () => {
    try {
      ...
      setSentryStatus(`Gagal mengirim metric: ${error instanceof Error ? error.message : String(error)}`);
    }
  };
